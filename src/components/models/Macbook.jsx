...
import modelUrl from "../../assets/models/macbook-transformed.glb";
...
export default function MacbookModel(props) {
  ...
  const texturePath = useMacbookStore((state) => state.texture);
  const color = useMacbookStore((state) => state.color);
  const screen = useVideoTexture(texturePath);
  ...
  return (
    <group {...props} dispose={null}>
      ...
      {/* ✅ Only this part changed — ensure the video updates */}
      <mesh geometry={nodes.Object_123.geometry} rotation={[Math.PI / 2, 0, 0]}>
        <meshStandardMaterial key={texturePath} map={screen} />
      </mesh>
